    ...
    public void Generate() {
        for(int y = -Size; y <= Size; y++) {
            for (int x = -Size; x <= Size; x++) {
                ...
                SquareRoomController room = Instantiate(_roomPrefab, position, Quaternion.identity, _roomHolder).GetComponent<SquareRoomController>();
                room.GenerationPointGridPos = new Vector2Int(x, y);
            }
        }
    }
